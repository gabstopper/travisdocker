...
class NodeTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        inject_mock_for_smc()
        cls.node = Node(href='{}/node'.format(url))
        etag, data = cls.node_cache()
        cls.node.data = SimpleElement(etag=etag, **data)
    ...
    def _del_link_from_cache(self, link):
        self.node.data['link'] = [x for x in self.node.data['link']
                                  if x.get('rel') != link]
            
    
    def test_node_create(self, m):
        node = Node._create('mynode', 'virtual_fw_node')
    ...
    def test_node_using_meta(self, m):
        node = Node(href='{}/node'.format(url), type='virtual', name='mynode')
        _, data = NodeTests.node_cache()
        node.data = SimpleElement(**data)
    ...
    def test_cancel_unbind(self, m):
        ...
        self.assertRaises(
            LicenseError, lambda: self.node.cancel_unbind_license())
    ...
    def test_go_online_offline_standby_lock(self, m):
        ...
        for action in actions:
            ...
            self.assertRaises(NodeCommandFailed,
                              lambda: getattr(self.node, action)())
    ...
    def test_time_sync(self, m):
        ...
        self._del_link_from_cache('time_sync')
        self.assertRaises(NodeCommandFailed, lambda: self.node.time_sync())
    ...
    def test_reset_user_db(self, m):
        ...
        # Some nodes do not have this attribute (i.e. virtual nodes)
        self._del_link_from_cache('reset_user_db')
        self.assertRaises(NodeCommandFailed, lambda: self.node.reset_user_db())
    ...
    def test_diagnostics(self, m):
        ...
        self._del_link_from_cache('diagnostic')
        self.assertRaises(NodeCommandFailed, lambda: self.node.diagnostic())
    ...
    def test_appliance_status(self, m):
        ...
        self.assertRaises(NodeCommandFailed,
                          lambda: self.node.appliance_status)
    ...
    def test_change_ssh_pwd(self, m):
        ...
        self.assertRaises(NodeCommandFailed,
                          lambda: self.node.change_ssh_pwd('pwd'))
        #del self.node._resource.change_ssh_pwd
        self._del_link_from_cache('change_ssh_pwd')
        # Not supported
        self.assertRaises(NodeCommandFailed,
                          lambda: self.node.change_ssh_pwd('pwd'))
    ...
    def test_initial_contact(self, m):
        ...
        # As file
        self.assertRaises(NodeCommandFailed,
                          lambda: self.node.initial_contact(filename='/foo'))

        # As base 64
        b64 = self.node.initial_contact(as_base64=True)
        import base64
        self.assertTrue(base64.b64decode(b64) == b'INITIAL_CONFIG')
                
        # Doesn't exist for certain engines (virtual)
        self._del_link_from_cache('initial_contact')
        self.assertRaises(NodeCommandFailed,
                          lambda: self.node.initial_contact())
    ...
    def test_diagnostic(self, m):
        ...
        self.assertRaises(NodeCommandFailed,
                          lambda: self.node.send_diagnostic([]))
