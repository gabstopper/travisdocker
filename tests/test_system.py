...
class Test(unittest.TestCase):
    ...
    def testLicense(self):
        ...
        self.assertTrue(len(system.licenses) >= 1)
        self.assertIsInstance(system.licenses[0], License)
    ...
    def testExportElements(self):
        system = System()
        export = system.export_elements(
                    filename='system-elements.zip',
                    typeof='foo',  # Will get all elements
                    )
        self.assertIsInstance(export, DownloadTask)
    ...
    def testVisbileVirtualEngineMapping(self):
        ...
        system = System()
        self.assertFalse(
            system.visible_virtual_engine_mapping().get('mapping'))
    ...
    def test_engine_upgrade_as_mock(self, m):
        ...
        upgrade = EngineUpgrade(href='{}/upgrade'.format(url))
        upgrade.data = SimpleElement(**upgrade_cache)
        ...
        self.assertTrue(upgrade.download().follower.startswith('http'))

        m.post('/activate', [{'status_code': 200, 'json': task_progress,
                              'headers': {'content-type': 'application/json'}}])

        self.assertTrue(upgrade.activate().follower.startswith('http'))
        ...
        m.get('/engine_upgrade', [{'status_code': 200,
                                   ...
                                   'headers': {'content-type': 'application/json'}}])
        smc_system = System()
        smc_system.data = SimpleElement(**system)
        for x in smc_system.engine_upgrade():
            self.assertIsInstance(x, EngineUpgrade)

    def test_active_alerts_ack_all(self):
        smc_system = System()
        if min_smc_version(6.2):
            result = smc_system.active_alerts_ack_all()
            self.assertIsNone(result)
        else:
            with self.assertRaises(ResourceNotFound):
                smc_system.active_alerts_ack_all()
