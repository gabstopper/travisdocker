...
from smc.api.common import fetch_href_by_name, fetch_entry_point
...
class Test(unittest.TestCase):
    ...
    def test_element_as_json_with_etag(self):
        ...
        href = smc.actions.search.element_as_json_with_etag(
            'test-common-api-user')
        self.assertIsNotNone(href.etag)
        self.assertEqual(href.json.get('name'), 'test-common-api-user')

        """ Test unknown host """
        href = smc.actions.search.element_as_json_with_etag(
            '1234test-common-api-user')
    ...
    def test_element_info_as_json(self):
        ...
        """ Test unknown host """
        href = smc.actions.search.element_info_as_json(
            '1234test-common-api-user')
        self.assertIsNone(href)
    ...
    def test_element_href_use_wildcard(self):
        ...
        """ Test unknown host """
        result = smc.actions.search.element_href_use_wildcard(
            '1234test-common-api-user')
    ...
    def test_element_href_use_filter(self):
        ...
        href = smc.actions.search.element_href_use_filter(
            'test-common-api-user', 'host')
        self.util_check_for_href(href)

        """ Test invalid user, valid filter """
        href = smc.actions.search.element_href_use_filter(
            '1234test-common-api-user', 'host')
        self.assertIsNone(href)

        """ Test invalid filter """
        href = smc.actions.search.element_href_use_filter(
            '1234test-common-api-user', 'host2')
        self.assertIsNone(href)
    ...
    def test_element_as_json_with_filter(self):
        result = smc.actions.search.element_as_json_with_filter(
            'test-common-api-user', 'host')
        self.assertEqual(result.get('name'), 'test-common-api-user')
    ...
    def test_element_href_by_batch_with_filter(self):
        elements = smc.actions.search.element_href_by_batch(
            ['test-common-api-user'], 'host')
        for x in elements:
            if x.get('test-common-api-user'):
                self.assertRegexpMatches(
                    x.get('test-common-api-user'), r'^http')
    ...
    def test_element_by_href_as_json(self):
        ...
        """ Bad href """
        self.assertRaises(SMCConnectionError, lambda: smc.actions.search.element_by_href_as_json(
            'http://wegwegwegw'))
    ...
    def test_element_by_href_as_smcresult(self):
        ...
        # check name field in json, should match
        self.assertEqual(element.json.get('name'), 'test-common-api-user')
    ...
    def test_element_name_by_href(self):
        result = smc.actions.search.element_info_as_json_with_filter(
            'test-common-api-user', 'host')
