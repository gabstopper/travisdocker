...
from smc.api.exceptions import EngineCommandFailed,\
    ...
    CertificateError, ElementNotFound
...
class Test(unittest.TestCase):
    ...
    def test_layer3_engine_methods(self):
        ...
        self.assertTrue(engine.modify_attribute(antivirus={'antivirus_enabled': False,
                                                             'antivirus_update': 'daily',
                                                             'antivirus_update_day': 'mo',
                                                             'antivirus_update_time': 21600000,
                                                             'virus_log_level': 'none',
                                                             'virus_mirror': 'update.nai.com/Products/CommonUpdater'}).startswith('http'))
        ...
        self.assertRaises(EngineCommandFailed, lambda: engine.add_route(
            '2.2.2.2', '10.10.10.0/22'))
        ...
        for sites in engine.internal_gateway.vpn_site.all():
            self.assertTrue(sites.name.startswith(
                'Automatic Site for smcpython-fw'))
    ...
    def test_virtual_engine_with_unsupported_methods(self):
        ...
        self.assertRaises(UnsupportedInterfaceType,
                          lambda: layer3ve.physical_interface)
        self.assertRaises(UnsupportedInterfaceType,
                          lambda: layer3ve.modem_interface)
        self.assertRaises(UnsupportedInterfaceType,
                          lambda: layer3ve.adsl_interface)
        self.assertRaises(UnsupportedInterfaceType,
                          lambda: layer3ve.wireless_interface)
        self.assertRaises(UnsupportedInterfaceType,
                          lambda: layer3ve.switch_physical_interface)
    ...
    def test_unsupported_internal_gw(self):
        ...
        self.assertRaises(UnsupportedInterfaceType,
                          lambda: engine.tunnel_interface)
