...
# In addition, it will test node when the node is not ready to verify the
# exception catching
...
class Test(unittest.TestCase):
    ...
    def test_engine_export(self):
        # Test iterating progress task
        self.assertIsInstance(engine.export(), DownloadTask)
    ...
    def test_engine_refresh_policy_failure(self):
        self.assertRaises(
            TaskRunFailed, lambda: engine_not_initialized.refresh())
    ...
    def test_engine_upload_failure(self):  # Bad Policy
        self.assertRaises(TaskRunFailed, lambda: next(
            engine.upload(policy='someboguspolicy')))
    ...
    def test_node_bind_license(self):
        for node in engine_not_initialized.nodes:
            ...
            self.assertRaises(
                LicenseError, lambda: node.cancel_unbind_license())
            # Cause LicenseError due to invalid license id specified
            self.assertRaises(
                LicenseError, lambda: node.bind_license(license_item_id='fooo'))

    def test_node_initial_contact_to_file(self):
        # Initial contact
        for node in engine_not_initialized.nodes:
            # Base level init, shown as text
            self.assertIsNotNone(node.initial_contact())
            result = node.initial_contact(
                filename='initial_contact.txt')  # Saved to file
            self.assertIsNotNone(result)
        import os
        self.assertTrue(os.path.isfile('initial_contact.txt'))
        cwd = os.getcwd()  # Force failure of initial contact
        self.assertRaises(NodeCommandFailed,
                          lambda: node.initial_contact(filename=cwd))

    def test_node_initial_contact_fail(self):
        # Fails on VE nodes for example
        for node in engine.nodes:
            self.assertRaises(NodeCommandFailed,
                              lambda: node.initial_contact())
    ...
    def test_node_appliance_status_fail(self):
        ...
        self.assertRaises(
            NodeCommandFailed, lambda: engine_not_initialized.nodes[0].appliance_status)
    ...
    def test_time_sync_failure(self):
        self.assertRaises(NodeCommandFailed,
                          lambda: engine_not_initialized.nodes[0].time_sync())
        self.assertRaises(NodeCommandFailed,
                          lambda: engine.nodes[0].time_sync())
    ...
    def test_ssh_enable_disable_fail(self):
        ...
        self.assertRaises(NodeCommandFailed,
                          lambda: engine_not_initialized.nodes[0].ssh())

    def test_ssh_change_pwd_fail(self):
        # Cant change pwd on uninit node or virtual engine
        self.assertRaises(
            NodeCommandFailed, lambda: engine.nodes[0].change_ssh_pwd(pwd='password'))
        self.assertRaises(
            NodeCommandFailed, lambda: engine_not_initialized.nodes[0].change_ssh_pwd(pwd='password'))

    def test_diagnostic(self):
        # Unsupported node type (virtual_fw)
        self.assertRaises(NodeCommandFailed,
                          lambda: engine.nodes[0].diagnostic())
        # Not initialized node
        self.assertRaises(NodeCommandFailed,
                          lambda: engine_not_initialized.nodes[0].diagnostic())

    def test_reboot_fail(self):
        # Not initialized node
        self.assertRaises(NodeCommandFailed,
                          lambda: engine_not_initialized.nodes[0].reboot())

    def test_engine_snapshot(self):
        # Existing engine will have at least one snapshot from initial policy
        # push
        ...
        self.assertRaises(EngineCommandFailed,
                          lambda: snapshots[0].download(filename='/'))
    ...
    def test_engine_routing_all(self):
        ...
        for route in routes:
            ...
            if route.name == 'Interface 1':
                for r in route.all():
                    self.assertEqual(r.ip, u'10.29.248.21/30')
